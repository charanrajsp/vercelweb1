...
router.post('/seller/login', async (req, res) => {
    try {
      ...
      // Find seller by sellerId and email/phone
      ...
      if (!seller) {
        return res.status(400).json({
          ...
          details: 'No seller found with the provided ID and email/phone'
        });
      }
  ...
  router.get('/seller/verify/:token', async (req, res) => {
    try {
      ...
      seller.verificationToken = null; // Clear token after verification
      await seller.save();
...
router.post('/add-product', async (req, res) => {
    try {
      ...
      if (!name || !price || !img || !category || !rating || !inStockValue) {
